...
function Person({
    ...
    hat = hats[hat];
    //console.log(hat);
    const weaponWidth = weapon.weaponWidth;
    const weaponHeight = weapon.weaponHeight;
    ...
    const weaponBox = Bodies.rectangle(x, y + 5, weaponWidth, weaponHeight, {
        collisionFilter: {
            mask: myMask,
            group: Body.nextGroup(false),
            category: myCategory
    ...
    head.health = 50 * strength;
    head.maxHealth = 50 * strength;
    torso.health = 100 * strength;
    torso.maxHealth = 100 * strength;
    [upperArm1, lowerArm1, upperArm2, lowerArm2, upperLeg1, lowerLeg1, upperLeg2, lowerLeg2].forEach(x => {
        x.health = 30 * strength;
        x.maxHealth = 30 * strength;
    })
    let inGame = false;
    let timeSurvived = 0;
    let torsoHeight = 0;
    ...
    return {
        ...
        draw() {
            ...
            if (inGame) {
                ...
                if (keyIsPressed && key === " " && this === player) {
                ...
                if (typeof type === "string" && type.startsWith("melee") && !player.deadBodyParts.includes(player.head)) {
                    ...
                    enemies.forEach(enemy => {
                        if (dist(this.x, this.y, enemy.x, enemy.y) < 200) {
                ...
                if (typeof type === "string" && type.startsWith("ranged") && !player.deadBodyParts.includes(player.head)) {
                    if (!this.deadBodyParts.includes(lowerArm1)) {
                        ...
                        const toMouse1 = vecTo(lowerArm1.position.x, lowerArm1.position.y, player.head.position.x, player.head.position.y, c);
                        const toMouse2 = vecTo(lowerArm2.position.x, lowerArm2.position.y, player.head.position.x, player.head.position.y, c);
                        const toMouseWeapon = vecTo(weaponBox.position.x, weaponBox.position.y, player.head.position.x, player.head.position.y, 1);
                        if (Math.random() < 0.02 && distToPlayer < 600) {
                            const bullet = Bodies.rectangle(weaponBox.position.x + toMouseWeapon.x * 10, weaponBox.position.y + toMouseWeapon.y * 10, 10, 3, {
                                frictionAir: 0,
                                angle: createVector(player.head.position.x - weaponBox.position.x, player.head.position.y - weaponBox.position.y).heading()
                            });
                            /* bullet.velocity.x = toMouseWeapon.x * 3000;
                             bullet.velocity.y = toMouseWeapon.y * 3000;*/
                            Body.setVelocity(bullet, { x: toMouseWeapon.x * 25, y: toMouseWeapon.y * 25 });
                            bullets.push(bullet);
                ...
                if (hat) {
                    ...
                    rotate(hat === hats.spaceHelmet && !this.deadBodyParts.includes(head) ? torso.angle : head.angle);
                    //tint(255, deadTimer);
        ...
        takeDamage(opponent) {
            ...
            Detector.collisions(this.bodyParts.map(x => [this.opponent.weapon, x]).concat(this.bodyParts.map(x => [x, this.opponent.weapon])), engine).forEach(x => {
                ...
                if (body.health) {
                    if (!bullets.includes(opponent)) {
                        body.health -= this.opponent.weapon.angularSpeed * 5 * this.opponent.strength;
                        healthLost += this.opponent.weapon.angularSpeed * 5 * this.opponent.strength;
                    } else {
                        const velocity = Math.hypot(body.velocity.x, body.velocity.y);
                        body.health -= velocity * 0.5 * this.opponent.strength;
                        healthLost -= velocity * 0.5 * this.opponent.strength;
                    }
                    if (body.health < 0) {
